...
import (
	...
	model2 "github.com/Vioneta/VionetaOS/model"
	"github.com/Vioneta/VionetaOS/model/notify"
...
func (i *notifyServer) SendFileOperateNotify(nowSend bool) {
	if nowSend {
		...
		if len == 0 {
			...
			response, err := MyService.MessageBus().PublishEventWithResponse(context.Background(), common.SERVICENAME, "vionetaos:file:operate", msg)
			if err != nil {
				logger.Error("failed to publish event to message bus", zap.Error(err), zap.Any("event", msg))
			}
			if response.StatusCode() != http.StatusOK {
